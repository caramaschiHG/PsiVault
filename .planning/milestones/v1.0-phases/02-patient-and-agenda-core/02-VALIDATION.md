...
| 02-01-03 | 01 | 1 | PATI-03 | unit/view-model | `pnpm vitest run tests/patient-summary.test.ts -t "patient operational summary fallback"` | ❌ W0 | ⬜ pending |
...
| 02-03-02 | 03 | 3 | PATI-03 | unit/view-model | `pnpm vitest run tests/patient-summary.test.ts -t "patient operational summary hydration"` | ❌ W0 | ⬜ pending |
| 02-03-03 | 03 | 3 | SCHD-07 | unit/view-model | `pnpm vitest run tests/appointment-defaults.test.ts -t "patient-context quick next-session" -t "completed-appointment quick next-session"` | ❌ W0 | ⬜ pending |
...
- [ ] `tests/patient-summary.test.ts` — operational summary fallback-state coverage in `02-01` plus scheduling-backed hydration coverage in `02-03`
...
- [ ] `tests/appointment-defaults.test.ts` — quick next-session coverage for both patient-context and completed-appointment entry points
...
| Patient profile reads identity first while still exposing operational context within the first screenful after scheduling data exists | PATI-03 | Information hierarchy and readability are visual judgments | Open a patient profile and confirm identity leads, with real next/last session and pending items plus document/finance states immediately below |
...
| Quick next-session carries forward useful defaults without silently assuming the next slot from both entry paths | SCHD-07 | Prefill usefulness and trust are interaction-level concerns | Start next-session from patient context and from a completed appointment action; confirm patient, duration, mode, and price prefill while date/time stay explicit in both cases |
