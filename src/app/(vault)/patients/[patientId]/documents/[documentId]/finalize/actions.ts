...
import { finalizeDocument } from "../../../../../../lib/documents/model";
...
export async function finalizeDocumentAction(formData: FormData): Promise<void> {
  ...
  try {
    ...
    if (!doc || doc.patientId !== patientId || doc.status !== "draft") {
      guardRedirect = true;
    } else {
      const now = new Date();
    ...
    console.error("[finalizeDocumentAction]", err);
    return;
  }
